...
namespace system10
{
    public partial class MainWindow : Window
    {
        ...
        private static readonly string reportFilePath = "report.txt";
        ...
        private void ThirdThreadWork()
        {
            ...
            {
                mutex.ReleaseMutex();
                Task.Run(() => FourthThreadWork());
            }
        }

        private void FourthThreadWork()
        {
            mutex.WaitOne();
            try
            {
                ...
                Dispatcher.Invoke(() =>
                {
                    OutputTextBox.AppendText("Четвертий потік завершив роботу. Підготовлено звіт та записано у файл.\n");
                    OutputTextBox.AppendText($"Звіт:\n{reportContent}");
                });
            }
            finally
            {
        ...
        private string GenerateReport()
        {
            ...
            report.Add(GenerateFileReport(firstFilePath));
            report.Add(GenerateFileReport(secondFilePath));
            report.Add(GenerateFileReport(thirdFilePath));

            return string.Join("\n\n", report);
        }

        private string GenerateFileReport(string filePath)
        {
            var lines = File.ReadAllLines(filePath);
            var fileInfo = new FileInfo(filePath);

            var report = $"Файл: {filePath}\n" +
                         $"Кількість чисел: {lines.Length}\n" +
                         $"Розмір файлу (байт): {fileInfo.Length}\n" +
                         $"Вміст файлу:\n{string.Join(", ", lines)}";
